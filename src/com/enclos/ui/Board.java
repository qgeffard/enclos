...
public class Board extends JPanel {
    ...
    private void drawSheep(Graphics2D g) {
        for (int i = 1; i < this.NB_SHEEP + 1; i++) {
            ...
            if (currentHexa != null) {
                Point2D centerOfHexa = new Point((int) currentHexa.getPointList().get(3).getX() + Math.round(currentHexa.getSize() / 2),
                        (int) currentHexa.getPointList().get(3).getY() - Math.round(currentHexa.getSize() / 2));
    ...
    private void drawCenterCell(Graphics2D g) {
        ...
        for (int i = 0; i < 6; i++) {
            Point point = new Point((int) (this.getWidth() / 2 + hexagons.get(0).getSize() * Math.cos(i * 2 * Math.PI / 6)),
                    (int) (this.getHeight() / 2 + hexagons.get(0).getSize() * Math.sin(i * 2 * Math.PI / 6)));
                        ...
                        if (Board.this.firstHexSelected == null) {
                            ...
                            if (selectedHexagon.getNeighboors().contains(hex)) {
                                if (hex.getSheep() == null) {
                                    ...
                                    && hex.getSheep().getOwner().equals(Board.this.currentPlayer)) {
                                Board.this.firstHexSelected = hex;
                                Speaker.playRandomSaySheep();
                                resetHexagonsColor();
                                colorNeighboors(hex);
    ...
    private void nextTurn() {
        ...
        this.nbTurn++;
        this.currentPlayer = this.playerList.get(this.nbTurn % this.playerList.size()); // Get each player one by one
