    ...
    private void addNewGameItemListener(JMenuItem newGameItem) {
        ...
        newGameItem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                /*
                 * Map<String, String> settings = NewGameForm.display(); // si on a pas fait cancel if (settings != null) { String boardSize =
                 * settings.get("boardSize"); // on cr�e le board qui va bien Board newGame = new Board(Integer.valueOf(boardSize), 3); boolean close =
                 * settings.get("close").equals("close") ? true : false; // si on a decid� de close les autre jeux if (close) { // on supprime les autres jeux
                 * EnclosMenu.this.parent.getFrameContentPane().resetGamePanel(); EnclosMenu.this.parent.getFrameContentPane().addToGamePanel(newGame); } else {
                 * // sinon on ajoute le jeu apr�s les autres EnclosMenu.this.parent.getFrameContentPane().addToGamePanel(newGame); } }
                 * 
                 * }
                 * 
                 * });
